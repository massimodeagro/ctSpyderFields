...
By default, focal_point_type='sphere'. This means that a sphere will be fit on the lens surface, and its center will be used as the focal point. It can happen that the lens is very flat, or maybe the point cloud is of bad quality. This can cause the sphere fitting algorithm to fail, and generate over or undersized sphere with centers behind the retina. To help in the selection, the function will output for each computed eye 2 goodness of fit scores, together with the associated residuals. At first, the distance from each voxel and the ideal sphere is calculated, and the square of the values is averaged to provide a deviation score `rms_sphere`. This is then compared to `sigma=1`, assumed to be the expected error given discrete unitary voxels over a continuous sphere, to provide a 0-1 score `surface_score`, where 0 is no fit, 1 as perfect as possible fit. For the second goodness of fit score, we repeat the same residual calculation process for an aligned plane (`rms_plane`), to check whether the computed sphere is actually better than a simple plane (i.e., is the lens actually a sphere cap or not?). Then, we `rms_sphere/rms_plane`, subract from 1 and cap to 0-1, to provide a `sphericality` score. In any case, if the focal point as calculated by the sphere is not appropriate for your specimen can be noticed if when projecting the FOV they appear flipped or otherwise unrealistic (we will see this in the next step).

If any of this happens, you can use focal_point_type='given'. In this case, the focal point will be defined as a point along the lens main axis, equidistant between the top surface of the lens and the bottom surface of the retina. You can manually shift this point more towards the lens or the retina by changing focal_point_position value. here, 0 = in contact with the retinal surface, 1 = in contact with the lens surface, 0.5 = midway between the two, all other values beween 0 and 1 = proportional positioning. For this calculation, the software will also provide goodness of fit scores, which may help you guide in deciding the focal point distance from retina/lens.
